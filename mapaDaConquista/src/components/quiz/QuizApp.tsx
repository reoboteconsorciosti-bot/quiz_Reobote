...
export function QuizApp() {
  ...
  const [scores, setScores] = useState<Record<ProfileKey, number>>({
    executor: 0, analista: 0, guardiao: 0, visionario: 0, construtor: 0,
  });
  ...
  const handleSelect = (idx: number) => {
    ...
    setTimeout(() => {
      ...
      if (step + 1 < QUESTIONS.length) {
        ...
        // Enviar os dados completos para o backend agora que temos o perfil
        sendDataToBackend(primary);
      }
    }, 550);
  };

  const sendDataToBackend = async (finalProfile: ProfileKey) => {
    if (!name || !whatsapp || !objective) return;
    ...
    try {
      ...
      await fetch("/api/webhook", {
        ...
        body: JSON.stringify({
          ...
          telefone: finalPhone,
          perfil: finalProfile,
          objetivo: objective.toLowerCase(),
          tag: "quiz",
        }),
      });
    } catch (error) {
    ...
    <div className="relative min-h-screen w-full overflow-hidden bg-aurora">
      ...
      <main className="relative z-10 mx-auto flex min-h-[calc(100vh-90px)] max-w-3xl items-center justify-center px-5 pb-10 sm:px-8">
        <AnimatePresence mode="wait">
          ...
          {stage === "success" && <Success key="success" primary={primary} />}
        </AnimatePresence>
      </main>
    </div>
  );
...
/* ---------- REVEAL ---------- */
function Reveal({
  primary, secondary, isHybrid, compatibility, onContinue,
}: { primary: ProfileKey; secondary: ProfileKey; isHybrid: boolean; compatibility: number; onContinue: () => void }) {
...
/* ---------- LEAD ---------- */
function LeadForm({
  name, setName, whatsapp, setWhatsapp, objective, setObjective, onSubmit, onReset, isSubmitting,
}: {
  name: string; setName: (v: string) => void;
  whatsapp: string; setWhatsapp: (v: string) => void;
  objective: string; setObjective: (v: typeof objectives[number]) => void;
  ...
  return (
    ...
    >
      ...
      <form onSubmit={onSubmit} className="mx-auto mt-6 max-w-md space-y-4 rounded-3xl glass p-5 sm:p-6 shadow-xl">
        ...
        <div>
          <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Objetivo</p>
          <div className="grid grid-cols-2 gap-2">
            {objectives.map((o) => {
              const active = objective === o;
              return (
                <button type="button" key={o} onClick={() => setObjective(o)}
                  className={`option-card !p-3 text-sm font-medium ${active ? "selected" : ""}`}
                >
                  {o}
                </button>
              );
...
/* ---------- SUCCESS ---------- */
function Success({ primary }: { primary: ProfileKey }) {
  const p = PROFILES[primary];
  return (
    <motion.section
